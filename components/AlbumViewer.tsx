...
export default function AlbumViewer({ image, title, index, total, previousHref, nextHref, pages }: Props) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [modalIndex, setModalIndex] = useState(index - 1);
  ...
  const activePage = pages[modalIndex] ?? pages[index - 1];

  const move = useCallback((delta: number) => {
    setZoom(1);
    setModalIndex((current) => Math.min(pages.length - 1, Math.max(0, current + delta)));
  }, [pages.length]);

  const closeViewer = useCallback(() => {
    setOpen(false);
    if (modalIndex !== index - 1 && activePage) router.push(activePage.href);
  }, [activePage, index, modalIndex, router]);

  useEffect(() => {
    if (!open) setModalIndex(index - 1);
  }, [index, open]);
  ...
  }, [nextHref, open, previousHref, router]);
  ...
  function stopDrag(event: React.PointerEvent<HTMLDivElement>) {
    ...
    if (swipe && zoom === 1) {
      ...
      if (Math.abs(deltaX) >= 60 && Math.abs(deltaX) > Math.abs(deltaY) * 1.25) {
        if (deltaX < 0 && modalIndex < pages.length - 1) move(1);
        if (deltaX > 0 && modalIndex > 0) move(-1);
      }
    }
  ...
  return (
    <>
      <button type="button" onClick={() => { setModalIndex(index - 1); setZoom(1); setOpen(true); }} className="group relative block w-full cursor-zoom-in overflow-hidden border border-ink/14 bg-[#d6cfbf] text-left shadow-[0_22px_75px_rgba(31,29,24,0.14)]" aria-label={`放大查看${title}`}>
      ...
      {open && activePage && (
        <div className="fixed inset-0 z-[100] flex flex-col bg-[#111614]/96 text-white" role="dialog" aria-modal="true" aria-label={`${activePage.title}放大阅读`}>
          <div className="flex min-h-14 items-center justify-between gap-3 border-b border-white/12 px-3 md:px-5">
            <div className="min-w-0"><p className="truncate font-serif-cn text-sm font-bold md:text-base">{activePage.title}</p><p className="text-[11px] text-white/48">第 {modalIndex + 1} 页 / 共 {total} 页</p></div>
            <div className="flex items-center gap-1.5 text-xs">
              ...
              <button type="button" onClick={closeViewer} className="ml-1 h-9 border border-white/18 px-3 hover:bg-white/10">关闭</button>
            </div>
          </div>
          ...
          <div ref={viewportRef} className={`album-pan touch-none flex-1 overflow-auto ${zoom > 1 ? "cursor-grab active:cursor-grabbing" : ""}`} onPointerDown={startDrag} onPointerMove={drag} onPointerUp={stopDrag} onPointerCancel={cancelDrag}>
            <div className="flex min-h-full min-w-full items-center justify-center p-2 md:p-5" style={{ width: `${zoom * 100}%` }}>
              <img src={activePage.image} alt={`${activePage.title}完整页放大图`} width={5040} height={2160} draggable={false} className="block h-auto w-full max-w-none select-none" onDoubleClick={() => changeZoom(zoom === 1 ? 2 : 1)} />
            </div>
          </div>

          <div className="flex min-h-14 items-center justify-between border-t border-white/12 px-3 text-xs md:px-5">
            {modalIndex > 0 ? <button type="button" onClick={() => move(-1)} className="px-3 py-2 hover:text-[#b8d4e8]">← 上一页</button> : <span className="px-3 py-2 text-white/24">← 上一页</span>}
            ...
            {modalIndex < pages.length - 1 ? <button type="button" onClick={() => move(1)} className="px-3 py-2 hover:text-[#b8d4e8]">下一页 →</button> : <span className="px-3 py-2 text-white/24">下一页 →</span>}
          </div>
        </div>
      )}
    </>
