# 纸上山河 — 邮票·画册·名画 总站（方寸集为其中邮票馆）

- **网站已升级为"总门厅 + 三馆"架构**（2026-07-21 起）：`/` 是总门厅"纸上山河"，`/stamps` 是方寸集邮票馆（原来的整个网站），`/gallery`（中外名画）、`/albums`（画册）是筹备中的占位页。**架构决策、路由结构、视觉设计规矩（配色/印章/书签样式）全部记录在 `三馆架构说明.md`，改门厅或做画册馆/名画馆之前必须先看这份文档**，避免重复踩已经踩过的坑。
- 产品需求见 `PRD.md`（**注意 PRD.md 目前只覆盖方寸集邮票馆，三馆架构的背景和规划以 `三馆架构说明.md` 为准**）；架构：Next.js 15 + Tailwind v4，无数据库，数据在 `data/stamps/*.json`（按年份分文件），收藏状态存 localStorage。
